...
class Booking extends REST_Controller {
    ...
    public function booking_list_post(){
        ...
        if(REST_Controller::TOKEKEYS != $token){
            ...
            if($Load_status=='0' OR ($Load_status != '' OR !empty($Load_status))) {
                ...
                if($LoadID){
                    $qu = 1;
                    $this->db->select('tbl_booking.*,tbl_users.name as BookedByName,tbl_users.mobile as BookedByMobile, tbl_company.CompanyName, tbl_opportunities.OpportunityName,tbl_opportunities.Street1,IFNULL(tbl_opportunities.Street2, "") as Street2,tbl_opportunities.Town,tbl_opportunities.County,tbl_opportunities.PostCode,tbl_contacts.ContactName,tbl_materials.MaterialName, tbl_booking_loads.LoadID,tbl_booking_loads.TipID,tbl_tipaddress.TipName,tbl_booking_loads.CustomerName,tbl_booking_loads.Signature,tbl_booking_loads.TipNumber,tbl_booking_loads.GrossWeight as blGrossWeight,tbl_booking_loads.Tare as blTare,tbl_booking_loads.Net as blNet, tbl_booking_loads.Status as Load_status,tbl_booking_loads.Notes as loadNotes,tbl_booking_loads.Notes1 as loadNotes1,tbl_booking_loads.Notes2 as loadNotes2,tbl_booking_loads.JobStartDateTime,tbl_booking_loads.ConveyanceNo,tbl_booking_loads.JobEndDateTime,tbl_booking_loads.ReceiptName,tbl_booking_loads.AllocatedDateTime');
                    ...
                    $this->db->join(' tbl_users', 'tbl_booking.BookedBy =   tbl_users.userId');
            ...
            if($query->num_rows() > 0){
                ...
                foreach($dataarr as $dataar){
                  ...
                  if($bd == date('d-m-Y')){
                    ...
                    $this->db->from('tbl_booking_loads');
                    $this->db->where('tbl_booking_loads.DriverID', $DriverID);
                    $this->db->where_in('tbl_booking_loads.Status', [1,2,3]);
    ...
    public function booking_uncomplicated_list_post(){
        ...
        if(REST_Controller::TOKEKEYS != $token){
            ...
            $this->db->select('tbl_booking.*, tbl_company.CompanyName, tbl_opportunities.OpportunityName,tbl_opportunities.Street1,IFNULL(tbl_opportunities.Street2, "") as Street2,tbl_opportunities.Town,tbl_opportunities.County,tbl_opportunities.PostCode,tbl_contacts.ContactName,tbl_materials.MaterialName, tbl_booking_loads.LoadID,tbl_booking_loads.TipID,tbl_tipaddress.TipName,tbl_booking_loads.CustomerName,tbl_booking_loads.TipNumber,tbl_booking_loads.Status as Load_status,tbl_booking_loads.JobStartDateTime,tbl_booking_loads.ConveyanceNo,tbl_booking_loads.JobEndDateTime,tbl_booking_loads.AllocatedDateTime');
            $this->db->from('tbl_booking_loads');
            $this->db->join('tbl_booking', 'tbl_booking_loads.BookingID = tbl_booking.BookingID');
            $this->db->join('tbl_company', 'tbl_booking.CompanyID = tbl_company.CompanyID');
            $this->db->join('tbl_opportunities', 'tbl_booking.OpportunityID = tbl_opportunities.OpportunityID');
            ...
            $this->db->join(' tbl_tipaddress', 'tbl_booking_loads.TipID =   tbl_tipaddress.TipID');
            $this->db->where('tbl_booking_loads.DriverID', $DriverID);
            $this->db->where('tbl_booking_loads.Status != ',0,FALSE);
            $this->db->where('tbl_booking_loads.Status != ',4,FALSE);
            $this->db->where('tbl_booking_loads.Status != ',5,FALSE);
            $this->db->where('tbl_booking_loads.Status != ',6,FALSE);
                ...
                foreach($dataarr as $dataar){
                    ...
                    $LBookingID =$dataar->BookingID;
                     if($dataar->BookingType == 2){
                        $loadNo = $this->db->query("select * from tbl_booking_loads where BookingID = '$LBookingID'");
 ...
 'tbl_drivers_login.Status' => 0
        ...
        if(REST_Controller::TOKEKEYS != $token){
            ...
            $this->db->select('tbl_booking.*,tbl_company.CompanyName,tbl_opportunities.OpportunityName,tbl_materials.MaterialName, tbl_materials.SicCode, tbl_opportunities.Street1,IFNULL(tbl_opportunities.Street2, "") as Street2,tbl_opportunities.Town,tbl_opportunities.County,tbl_opportunities.PostCode, tbl_booking_loads.LoadID,tbl_booking_loads.LID,tbl_booking_loads.TicketID,tbl_booking_loads.TicketUniqueID,tbl_booking_loads.ConveyanceNo,tbl_booking_loads.BookingID,tbl_booking_loads.MaterialID,tbl_booking_loads.TipID,tbl_booking_loads.DriverName,tbl_booking_loads.VehicleRegNo,tbl_booking_loads.TicketID,tbl_booking_loads.TicketUniqueID,tbl_booking_loads.CustomerName,tbl_booking_loads.Signature as DriverSign, tbl_booking_loads.Status as Load_status, tbl_booking_loads.SiteInDateTime, tbl_booking_loads.SiteOutDateTime');
            $this->db->from('tbl_booking_loads');
            $this->db->join('tbl_booking', 'tbl_booking_loads.BookingID = tbl_booking.BookingID');
            $this->db->join('tbl_company', 'tbl_booking.CompanyID = tbl_company.CompanyID');
            $this->db->join('tbl_opportunities', 'tbl_booking.OpportunityID = tbl_opportunities.OpportunityID');
            ...
            if($query->num_rows() > 0){
                ...
                if($Load_status == 1){
                    ...
                    if(empty($_FILES['Signature']['name']) AND $dataarr[0]->BookingType == 1 AND !$wastedJourney){
                        ...
                        $Booking = $this->Booking_API_Model->GetBookingInfo($BookingID); 
                        ...
                        if($dataarr[0]->BookingType == 1){
                          //echo $dataarr[0]->BookingType.'-'.$TicketUniqueID.'-'.$TicketNumber; exit();
                          //print_r($user) ; 
                          //echo $user['Tare'];
                          //exit();
                          if($dataarr[0]->TipID == 1){
                            ...
                            /*$ticketsInfo = array('TicketUniqueID'=>$TicketUniqueID, 'LoadID'=>$LoadID, 'TicketNumber'=>$TicketNumber, 
                                ...
                                'IsInBound'=>$IsInBoundC,
                                'pdf_name'=>$TicketUniqueID.'.pdf',
                                'driversignature'=>$user['ltsignature']); 	*/
                        ...
                        if($dataarr[0]->BookingType == 2){
								...
								//$data1['tickets'] = $this->Common_model->get_pdf_data($ticketId); 
        						//$data1['tickets1'] = array( 
        						//    "driversignature1"=>base_url().'uploads/Signature/'.$SignatureUploadfile_name,
        						//    "CompanyName1"=>$CustomerName,
        						//    "ds"=>$data1['tickets']['driversignature']
        						//    );
        						//$html1=$this->load->view('Tickets/ticket_pdf_out1', $data1, true);
								if($wastedJourney){
                                    if($dataarr[0]->TipID == 1){    
								...
								//$html=$this->load->view('Tickets/ticket_pdf_out1', $data1, true);
        						//$mpdf01 =  new mPDF01('utf-8', array(70,210));
                                //$mpdf01->AddPage('P','','','','',5,5,5,5,5,5);
                                //$mpdf01->keep_table_proportions = false;
                                //$mpdf01->WriteHTML($html1);
                                //$mpdf01->Output($pdfFilePath1);
        						//pdf_out_1 end
                        } 
                        ...
                        if($dataarr[0]->BookingType == 2){
                            if($dataarr[0]->TipID == 1){
                                ...
                                //$this->db->query("update tbl_tickets set GrossWeight = '$GrossWeight',Net = $net,pdf_name = '$pdf_nameG' where LoadID = '$LoadID'");
                    ...
                    $LastConNumber =  $this->Booking_API_Model->LastConNumber(); 
                    ...
                    if($dataarr[0]->LoadType == 2 AND $finish_load_button != 1){    
						...
						##################### Insert Booking Date ###################
						$cndate = date('Y-m-d');
						$insertBookingDate = $this->db->query("insert into tbl_booking_date(BookingID,BookingDate) value('$LBookingID','$cndate')");
						$insertBookingDateID = $this->db->insert_id();
						
						##############################################################
						
						$dloginid = $user['DriverID'];
						$insertLoad = $this->db->query("insert into tbl_booking_loads(LID,TicketID,TicketUniqueID,ConveyanceNo,BookingID,BookingDateID,MaterialID,TipID,DriverID,DriverLoginID,DriverName,VehicleRegNo,Status,JobStartDateTime,AllocatedDateTime) value 
						('$ILID','$ITicketID','$LTicketUniqueID','$LConveyanceNo','$LBookingID','$insertBookingDateID','$LMaterialID','$LTipID','$DriverID',$dloginid,'$LDriverName','$LVehicleRegNo',0,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)");
						$NewLoadID = $this->db->insert_id(); 
						
						//$insertLoad = $this->db->query("insert into tbl_booking_loads(LID,TicketID,TicketUniqueID,ConveyanceNo,BookingID,BookingDateID,MaterialID,TipID,DriverID,DriverLoginID,DriverName,VehicleRegNo,Status,JobStartDateTime,AllocatedDateTime) value 
						//('$ILID','$ITicketID','$LTicketUniqueID','$LConveyanceNo','$LBookingID','$insertBookingDateID','$LMaterialID','$LTipID','$DriverID','$LDriverName','$LVehicleRegNo',0,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)");
						//$NewLoadID = $this->db->insert_id(); 
            ...
            if($query->num_rows() > 0){
                    ...
                    for($i = 0; $i < $filesCount; $i++){ 
                        ...
                        if($this->upload->do_upload('file')){ 
                           ...
                           $DriverLoginID = $user['DriverID'];
                           $this->db->query("INSERT INTO `tbl_booking_loads_photos` (`PhotoID`, `LoadID`, `DriverID`,`DriverLoginID`, `ImageName`, `CreateDateTime`, `UpdateDateTime`) VALUES 
