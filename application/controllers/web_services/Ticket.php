    ...
    public function printGenerate_post(){
        ...
        if(REST_Controller::TOKEKEYS != $token){
            ...
            if($query->num_rows() > 0){
               ...
               /* $this->db->select('TicketUniqueID');
                $this->db->from('tbl_tickets');
                $this->db->where('LoadID', $LoadID);
                $tquery = $this->db->get();
                $tdata = $query->result();
                if($tquery->num_rows() > 0 AND $tdata[0]->TicketUniqueID AND $dataarr[0]->BookingType == 2 AND $dataarr[0]->Load_status <= 3){
                    $pdf_file = base_url().'/assets/pdf_file/'.$tdata[0]->TicketUniqueID;
                } else {
                    $pdf_file = base_url().'/assets/conveyance/'.$dataarr[0]->ReceiptName;
                } */
