...
class Tickets extends BaseController
{
	...
	function AddConveyanceTicketAJAX()
	{
		...
		$SiteLogInfo = array(
			...
			'SitePage' => 'add inticket search conveyance',
		...
		$SiteLogInfo = array(
			...
			'UpdatedByUserID' => $this->session->userdata['userId'],
			'SitePage' => 'add inticket search conveyance pdf',
			'RemoteIPAddress' => $_SERVER['REMOTE_ADDR'],
			'BrowserAgent' => getBrowserAgent(),
			'AgentString' => $this->agent->agent_string(),
			'UserPlatform' => $this->agent->platform()
	...
	function EditConveyanceTicketAJAX()
	{
		...
		if (LoadID > "0" && $TicketNo > "0") {
			...
			$SiteLogInfo = array(
				...
				'UpdatedValue' => $LIJson . " => " . $coJson,
				'UpdatedByUserID' => $this->session->userdata['userId'],
				'SitePage' => 'edit inticket link conveyance',
				'RemoteIPAddress' => $_SERVER['REMOTE_ADDR'],
				'BrowserAgent' => getBrowserAgent(),
				'AgentString' => $this->agent->agent_string(),
				'UserPlatform' => $this->agent->platform()
	...
	function ViewInTicket($TicketID)
	{
		...
		if ($this->isView == 0) {
			...
			if ($data['TicketInfo']['TypeOfTicket'] != "In") {
				redirect('All-Tickets');
			}
	...
	function ViewOutTicket($TicketID)
	{
		...
		if ($this->isView == 0) {
			...
			if ($data['TicketInfo']['TypeOfTicket'] != "Out") {
				redirect('All-Tickets');
			}
	...
	function ViewCollectionTicket($TicketID)
	{
		...
		if ($this->isView == 0) {
			...
			if ($data['TicketInfo']['TypeOfTicket'] != "Collection") {
				redirect('All-Tickets');
			}
	...
	function AddOfficeTicketAJAX()
	{
		if ($this->isAdd == 0) {
			...
			if ($lastdate['TicketDate']) {
				$TicketDate = $lastdate['TicketDate'];
			} else {
				$TicketDate  = date('Y-m-d H:i:s');
			...
			$ticketsInfo = array(
				...
				'RegNumber' => $VechicleRegNo,
				'Hulller' => $HaullerRegNo,
				'MaterialID' => $DescriptionofMaterial,
...
//print_r($SicCode); exit;
			...
			if ($TicketNo != "" && trim($OpportunityID) != "" && trim($DescriptionofMaterial) != "" && trim($CompanyID) != "" && trim($driverid) != "") {
				...
				if ($tupdate == 1) {
					...
					if (file_exists($pdfFilePath)) {
						unlink($pdfFilePath); // replance new with existing 
					}
					//load mPDF library
					$this->load->library('m_pdf');
		...
		if ($this->isEdit == 0) {
			...
			if ($LorryNo == 0) {
				...
				$DriverInfo = array(
					...
					'RegNumber' => $VechicleRegNo,
					'Tare' => $Tare,
					'Haulier' => $HaullerRegNo,
					'ltsignature' => $driversignature,
					'CreateUserID' => $this->session->userdata['userId'],
					'CreateDate' => date('Y-m-d H:i:s'),
					'EditUserID' => ''
				);
			...
			if (trim($OpportunityID) != "" && trim($DescriptionofMaterial) != "" && trim($CompanyID) != "" && trim($driverid) != "") {
				...
				if ($tupdate == 1) {
					...
					//download it.
					$this->m_pdf->pdf->Output($pdfFilePath, "F");
					redirect(base_url($openPath)); // Redirect to the saved PDF file
				//	echo base_url($openPath);
				} else {
					echo "SAME";
				}
	...
	function CheckDuplicateTicket()
	{
		...
		if ($result['duplicate'] > 0) {
			...
			echo (json_encode(array('status' => TRUE, 'status1' => $result['duplicate'])));
		}
	...
	function AJAXUpdateGrossWeight()
	{
		if ($this->isEdit == 0) {
			...
			if ($result > 0) {
				echo (json_encode(array('status' => TRUE, 'TicketNo' => $TicketNo)));
			} else {
				echo (json_encode(array('status' => FALSE)));
			}
		}
