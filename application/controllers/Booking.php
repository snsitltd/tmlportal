    ...
    function ConveyanceExcelExportNew() {
        if ($this->isEdit == 0) {
            ...
            foreach ($data['SplitExcelDelTickets'] as $row) {
                ...
                if($row['MStatus'] == 1){
						$LT = '';
						if($row['LorryType'] == 1) { $LT = 'Tipper'; }
						else if($row['LorryType'] == 2) { $LT = 'Grab'; }
						else if($row['LorryType'] == 3) { $LT = 'Bin'; }
						else{ $LT = ''; }

						$TB = '';
						if($row['TonBook']== 1) { $TB = ' Tonnage '; } 
						else{ $TB = ' Load '; }
        ...
        if($this->isEdit == 0){
			...
			$this->load->library("excel");
				...
				foreach( $data['SplitExcelConveyanceTickets'] as $row){ 	
					...
					if($row['WaitTime']==null){ $min = "N/A";  }else{ $min = $row['WaitTime']." Min";  } 
				...
				$cname = "";$oname = "";
					...
					if($data['SplitExcelConveyanceTickets'][$i]['TipID']==1){
						...
						if($data['SplitExcelConveyanceTickets'][$i]['TipTicketNo']!='' && $data['SplitExcelConveyanceTickets'][$i]['TipTicketNo']!='0'){
                            ...
                            $url = "http://193.117.210.98:8081/ticket/Supplier/" . rawurlencode($data['SplitExcelConveyanceTickets'][$i]['TipName'] . "-" . $data['SplitExcelConveyanceTickets'][$i]['TipTicketNo']) . ".pdf";
                            $object->getActiveSheet()->getCell('H' . $excel_row)->getHyperlink()->setUrl($url);
						} 
					} 
					 
					$object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, $data['SplitExcelConveyanceTickets'][$i]['PurchaseOrderNo'] );    
					if($row['MStatus'] == 1){
						$LT = '';
						if($data['SplitExcelConveyanceTickets'][$i]['LorryType'] == 1) { $LT = 'Tipper'; }
						else if($data['SplitExcelConveyanceTickets'][$i]['LorryType'] == 2) { $LT = 'Grab'; }
						else if($data['SplitExcelConveyanceTickets'][$i]['LorryType'] == 3) { $LT = 'Bin'; }
						else{ $LT = ''; }

						$TB = '';
						if($data['SplitExcelConveyanceTickets'][$i]['TonBook']== 1) { $TB = ' Tonnage '; } 
						else{ $TB = ' Load '; }
					...
					if($data['SplitExcelConveyanceTickets'][$i]['BookingMaterialID'] == $data['SplitExcelConveyanceTickets'][$i]['MaterialID']){
                        $Price = is_numeric($data['SplitExcelConveyanceTickets'][$i]['Price']) ? $data['SplitExcelConveyanceTickets'][$i]['Price'] : 0;
                    }else{
                        $Price = 0;
                    } 		
					...
					$object->getActiveSheet()->setCellValueByColumnAndRow(10, $excel_row, $Price );     
					if($data['SplitExcelConveyanceTickets'][$i]['WaitTime']==null){ $min = "N/A";  }else{ $min = $data['SplitExcelConveyanceTickets'][$i]['WaitTime']." Min";  } 
					
					$object->getActiveSheet()->setCellValueByColumnAndRow(11, $excel_row, $min );    
					$object->getActiveSheet()->setCellValueByColumnAndRow(12, $excel_row, $data['SplitExcelConveyanceTickets'][$i]['Status'] );    
					if($NetWeight=='1'){ 
						//$object->getActiveSheet()->setCellValueByColumnAndRow(13, $excel_row, floor(($data['SplitExcelConveyanceTickets'][$i]['Net']/1000)*100)/100 );    
						
						//$NetWeight1 = round($data['SplitExcelConveyanceTickets'][$i]['Net']/1000,2);
						//$NetWeight1 = floor(($data['SplitExcelConveyanceTickets'][$i]['Net']/1000)*100)/100;
						
						$NetWeight1 = bcdiv($data['SplitExcelConveyanceTickets'][$i]['Net'],1000,2); 
						$object->getActiveSheet()->setCellValueByColumnAndRow(13, $excel_row,$NetWeight1);   
						
						//$TNetWeight = floor(($TNetWeight + $NetWeight1)*100)/100; 
						$TNetWeight = bcadd($TNetWeight,$NetWeight1,3);	 
						
						//$NPrice = floor(($NetWeight1*$data['SplitExcelConveyanceTickets'][$i]['Price'])*100)/100; 
						$NPrice = bcmul($NetWeight1,$data['SplitExcelConveyanceTickets'][$i]['Price'],3);
        ...
        if($this->isEdit == 0){
			...
			for($j=0;$j<count($Array);$j++){ 
				...
				$cname = "";$oname = "";
					...
					$object->getActiveSheet()->setCellValueByColumnAndRow(12, $excel_row, $data['SplitExcelConveyanceTickets'][$i]['Status'] );    
					if($NetWeight=='1'){ 
						$object->getActiveSheet()->setCellValueByColumnAndRow(13, $excel_row, round($data['SplitExcelConveyanceTickets'][$i]['Net']/1000,2) );   
						$NetWeight1 = floor(($data['SplitExcelConveyanceTickets'][$i]['Net']/1000)*100)/100;
						$TNetWeight = floor(($TNetWeight + $NetWeight1)*100)/100; 
						//$NetWeight1 = round($data['SplitExcelConveyanceTickets'][$i]['Net']/1000,2);
						//$TNetWeight = $TNetWeight + $NetWeight1;
						 
						$NPrice = floor(($NetWeight1*$data['SplitExcelConveyanceTickets'][$i]['Price'])*100)/100; 
						$TNPrice = floor(($TNPrice + $NPrice)*100)/100;
						//$NPrice = round($NetWeight1*$data['SplitExcelConveyanceTickets'][$i]['Price'],2); 
	...
	function WaitTimeSplitExcelExport(){
        if($this->isEdit == 0){
			...
			for($j=0;$j<count($Array);$j++){ 
				...
				//echo $Array[$j]; 
				if($j==0){ $newi = 0;  }else{ $newi = $Array[$j-1];  }
				$TPrice = 0; $ALLTotalWaitTime =0; $THours  = 0; 
				for($i=$newi; $i<$Array[$j]; $i++){ 
				//for($i=0;$i<count($data['SplitExcelConveyanceTickets']);$i++){  
				$cname = "";$oname = "";
					$url = ''; $TotalWaitTime = 0; 
	...
	function SplitExcelDelExport(){
        if($this->isEdit == 0){
			...
			for($j=0;$j<count($Array);$j++){ 
				...
				$object->setActiveSheetIndex(0);
				if($NetWeight=='1'){ 
					$table_columns = array("ConvTkt No","ConvTkt Date","Customer Name ", "Job Site Address","PO NO" ,"Product Description" ,"Price", "WaitTime","Status", "NetWeight", "Total Price"); 
				}else{
					$table_columns = array("ConvTkt No","ConvTkt Date","Customer Name ", "Job Site Address","PO NO" ,"Product Description" ,"Price","WaitTime", "Status"); 
				} 
				$column = 0; 
				foreach($table_columns as $field)
				{
				...
				for($i=$newi; $i<$Array[$j]; $i++){  
					...
					//$TPrice = round($TPrice + $Price,2);
					//$TPrice = floor(((float)$TPrice + (float)$Price)*100)/100;
					...
					$object->getActiveSheet()->setCellValueByColumnAndRow(7, $excel_row, $min );    
					$object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, $data['SplitExcelDelTickets'][$i]['Status']);    
					
					if($NetWeight=='1'){ 
						//$NetWeight1 = floor(($data['SplitExcelDelTickets'][$i]['Net']/1000)*100)/100; 
						$NetWeight1 = bcdiv($data['SplitExcelDelTickets'][$i]['Net'],1000,2); 
						...
						$NPrice = bcmul($NetWeight1,$data['SplitExcelDelTickets'][$i]['Price'],2);
				...
				$object->getActiveSheet()->setCellValueByColumnAndRow(6, $excel_row, $NPrice );   
				}
				if($NetWeight=='1'){ 
					$object->getActiveSheet()->getStyle('A'.$excel_row.':M'.$excel_row.'')->getFont()->setBold( true );		
        ...
        if($this->isEdit == 0){
			...
			for($j=0;$j<count($Array);$j++){ 
				...
				for($i=$newi; $i<$Array[$j]; $i++){  
					...
					if($NetWeight=='1'){ 
						$object->getActiveSheet()->setCellValueByColumnAndRow(9, $excel_row, round($data['SplitExcelDelTickets'][$i]['Net']/1000,2) );   
						//$NetWeight1 = round((float)$data['SplitExcelDelTickets'][$i]['Net']/1000,2); 
						$NetWeight1 = floor(((float)$data['SplitExcelDelTickets'][$i]['Net']/1000)*100)/100; 
						
						//$TNetWeight = (float)$TNetWeight + (float)$NetWeight1;	
						$TNetWeight = floor(((float)$TNetWeight + (float)$NetWeight1)*100)/100;	 
						
						//$NPrice = round((float)$NetWeight1*(float)$data['SplitExcelDelTickets'][$i]['Price'],2); 
						$NPrice = floor(((float)$NetWeight1*(float)$data['SplitExcelDelTickets'][$i]['Price'])*100)/100; 
						
						//$TNPrice = (float)$TNPrice + (float)$NPrice;
						$TNPrice = floor(((float)$TNPrice + (float)$NPrice)*100)/100;
						
						
						//$NetWeight1 = round($data['SplitExcelDelTickets'][$i]['Net']/1000,2);
						//$object->getActiveSheet()->setCellValueByColumnAndRow(9, $excel_row, round($data['SplitExcelDelTickets'][$i]['Net']/1000,2) );  
						//$TNetWeight = $TNetWeight + $NetWeight1;		
						 
						//$NPrice = round($NetWeight1*$data['SplitExcelDelTickets'][$i]['Price'],2); 
						//$TNPrice = $TNPrice + $NPrice;
						$object->getActiveSheet()->setCellValueByColumnAndRow(10, $excel_row, $NPrice );  
...
//exit;
	...
	function TipAddressUpdate($LoadID){
        if($this->isEdit == 0){
            ...
            if($LoadID  == null ){ redirect('ConveyanceTickets'); }       
			if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
				...
				if($TipID  == null ){ $this->session->set_flashdata('success', 'Tip Address Should Not Be Blank.');           redirect('ConveyanceTickets'); }      
				if($TipID  == '1'  ){ 
					if($TicketNumber == '' ){
				...
				if($update){ 	
					...
					'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
				'SitePage'=>'conveyance tip update','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
				...
				'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
							...
							'TicketDate'=>$TipTicketDatetime, 'OpportunityID'=>$data['LoadInfo']->OpportunityID, 'Conveyance'=>$data['LoadInfo']->ConveyanceNo, 
				...
				'SitePage'=>'conveyance date update','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
				'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
							$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
							/* ===================================== */
    ...
    function AJAXUpdateBookingPrice(){
        if($this->isEdit == 0){
			...
			$SiteLogInfo = array('TableName'=>'tbl_booking1' ,'PrimaryID'=>$BookingID, 
			'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
			'SitePage'=>'Update Price Grid ','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
			'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
			$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
			/* ========================================================================= */
			...
			$SiteLogInfo = array('TableName'=>'tbl_booking_request' ,'PrimaryID'=>$BookingRequestID, 
			'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
			'SitePage'=>'Update Price Grid','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
			'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
			$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
			...
			$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$BookingID, 
			'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
			'SitePage'=>'Update Price Grid','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
			'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
			$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
			...
			if($CountProductBookingID->CNT==0){
				...
				'BookingDateID'=>$BookingReqInfo[0]->BookingDateID,'BookingID'=>$BookingID ,'PurchaseOrderNo'=> $BookingReqInfo[0]->PurchaseOrderNo , 'Qty'=> $BookingReqInfo[0]->Loads , 
				...
				$SiteLogInfo = array('TableName'=>'tbl_product' ,'PrimaryID'=>$PID, 
				'UpdatedValue'=>$LoadInfoJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
				'SitePage'=>'Update Price Grid','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
				'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
				$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
				/* ========================================================================= */
				...
				$SiteLogInfo = array('TableName'=>'tbl_product' ,'PrimaryID'=>$BookingID, 
				'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
				'SitePage'=>'Update Price Grid','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
				'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
				$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
	...
	function AJAXUpdatePON(){
        if($this->isEdit == 0){
			...
			$SiteLogInfo = array('TableName'=>'tbl_booking1' ,'PrimaryID'=>$BookingID, 
			'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
			'SitePage'=>'Update Purchase Order Number','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
			'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
			$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
    ...
    function AJAXUpdateOpenPO(){
        if($this->isEdit == 0){
			...
			$SiteLogInfo = array('TableName'=>'tbl_booking1' ,'PrimaryID'=>$BookingID, 
			'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
			'SitePage'=>'Update OpenPO ','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
			'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
			$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
			
			if ($result) { echo(json_encode(array('status'=>TRUE,'BookingID'=>$BookingID))); }
            else { echo(json_encode(array('status'=>FALSE))); }
        ...
        if($this->isEdit == 0){
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
				...
				if($update){ 	
					...
					$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$LoadID, 
					'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
					'SitePage'=>'conveyance material update','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
					'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
					$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
					///* ===================================== 
					...
					if($PUpdate){
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$LoadID, 
						'UpdatedValue'=>$PInfoJson." => ".$PCondJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'conveyance material update','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
						// ===================================== /
				    ...
				    ///* =================== Site Logs ===================   
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$LoadID, 
						'FileName'=> $NewFileName , 'UpdatedByUserID'=>$this->session->userdata['userId'],
				'SitePage'=>'conveyance material update pdf','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
				'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
					// =====================================  
	...
	function MaterialUpdateDelivery($LoadID){
        if($this->isEdit == 0){
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
				...
				$SiteLogInfo = array('TableName'=>'tbl_tickets' ,'PrimaryID'=>TicketNo, 
				'UpdatedValue'=>$TInfoJson." => ".$TcondJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
				'SitePage'=>'delivery material update','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
				'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
				$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
				/* ===================================== */
				...
				if($update){   
				    ...
				    /* =================== Site Logs ===================  */
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$LoadID, 
						'FileName'=> $NewFileName , 'UpdatedByUserID'=>$this->session->userdata['userId'],
				'SitePage'=>'delivery material update pdf','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
				'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
					/* ===================================== */
	...
	function StatusUpdate($LoadID){
        if($this->isEdit == 0){
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
				...
				$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$LoadID, 
				'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
				'SitePage'=>'conveyance status update','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
				'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
				$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
				/* ===================================== */
				...
				if($update){  
					if($Status==4){
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$LoadID, 
						'FileName'=> $NewFileName , 'UpdatedByUserID'=>$this->session->userdata['userId'],
				'SitePage'=>'conveyance status update pdf','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
				'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
						/* ===================================== */
	...
	function DeliveryStatusUpdate($LoadID){
        if($this->isEdit == 0){
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
				...
				$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$LoadID, 
				'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
				'SitePage'=>'delivery status update','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
				'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
				$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
				/* ===================================== */
				...
				if($this->form_validation->run()){ 
				 ...
				 //exit;
					...
					if($OpportunityID == '0'){    
						...
						'Street1'=>$Street1,'Street2'=>$Street2,'Town'=>$Town ,'County'=>$County ,'PostCode'=>$PostCode,'Status'=>1 ); 
					...
					'WaitingCharge'=>$WaitingCharge , 'WaitingTime'=>$WaitingTime , 'PriceBy'=>$PriceBy ,  
					...
					if($BookingRequestID ){  
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_request' ,'PrimaryID'=>$BookingRequestID, 
						'UpdatedValue'=>$LoadInfoJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'Add Booking Request','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
						/* ========================================================================= */
						...
						for($i = 0; $i < count($BookingType); $i++ ){  
							if($BookingType[$i]!="" && $DescriptionofMaterial[$i]!=""  && $BookingDateTime[$i]!="" ){	
							     $OP = 0;
					             if(is_null($OpenPO[$i])){ $OP = 0; }else{ $OP =  $OpenPO[$i]; }
								...
								'Days' => '1' , 'Price' => $Price[$i], 'SICCode' => $SICCode[$i],'PurchaseOrderNo' => $PurchaseOrderNo[$i], 'OpenPO' =>$OP, 
								'TotalAmount' => $TotalHidden[$i] , 'BookedBy'=>$this->session->userdata['userId']); 
								$BookingID = '';
								...
								if($BookingID){ 
									...
									for($k=0;$k<count($BookingDate);$k++){ 	
										...
										if($insDate){
											...
											$SiteLogInfo = array('TableName'=>'tbl_booking_date1' ,'PrimaryID'=>$insDate, 
											'UpdatedValue'=>$LoadInfoJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
											'SitePage'=>'Add Booking Request','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
											'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
											$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
											...
											if($Price[$i]!='' && $Price[$i] !=0 ){ 
												if($k==0 ){ 
												$ProductInfo = array('OpportunityID'=>$OpportunityID, 'MaterialID'=>$DescriptionofMaterial[$i], 'SICCode'=>$SICCode[$i],  'BookingDateID'=>$insDate ,'BookingID'=>$BookingID ,
												'DateRequired'=>$BookingDate[$k] ,'UnitPrice'=>$Price[$i] , 'LorryType' => $LorryType[$i] ,'PurchaseOrderNo'=> $PurchaseOrderNo[$i] , 'Qty'=> $Loads[$i] , 'PriceType'=>'0' ,
												'CreateUserID'=>$this->session->userdata['userId'],'EditUserID'=>$this->session->userdata['userId']);         
												$PID = $this->Common_model->insert("tbl_product", $ProductInfo);	   
				...
				if($this->form_validation->run()){ 
				 ...
				 //exit;
					...
					if($BookingRequestID ){  
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_request' ,'PrimaryID'=>$BookingRequestID, 
						'UpdatedValue'=>$LoadInfoJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'Add Booking Request Tonnage','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
						/* ========================================================================= */
								...
								if($BookingID){ 
									...
									for($k=0;$k<count($BookingDate);$k++){ 	
										...
										if($insDate){
											...
											if($Price[$i]!='' && $Price[$i] !=0 ){ 
												if($k==0 ){ 
												$ProductInfo = array('OpportunityID'=>$OpportunityID, 'MaterialID'=>$DescriptionofMaterial[$i], 'SICCode'=>$SICCode[$i], 'BookingDateID'=>$insDate ,'PriceType'=>'1' ,
												'DateRequired'=>$BookingDate[$k] ,'UnitPrice'=>$Price[$i] ,'LorryType' => $LorryType[$i] ,'PurchaseOrderNo'=>$PurchaseOrderNo[$i] , 'Qty'=> $Loads[$i] , 'TotalTon' => $TotalTon[$i],
												'CreateUserID'=>$this->session->userdata['userId'],'EditUserID'=>$this->session->userdata['userId']);         
												$PID = $this->Common_model->insert("tbl_product", $ProductInfo);	 
				...
				if($this->form_validation->run()){ 
				 ...
				 //exit;
					...
					$isUpdate = $this->Common_model->update("tbl_booking_request",$BookingInfo, $cond);
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_request' ,'PrimaryID'=>$BookingRequestID, 
						'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'Edit Booking Request  ','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
						/* ========================================================================= */
						    ...
						    for($i = 0; $i < count($BArray); $i++ ){     
								...
								if($TotalLoadAllocated[$BArray[$i]]==0 ){	 
									if($BookingType[$BArray[$i]]!="" && $DescriptionofMaterial[$BArray[$i]]!=""  && $BookingDateTime[$BArray[$i]]!="" ){
									     ...
									     $OP = 0;
					                    if(is_null($OpenPO[$BArray[$i]])){ $OP = 0; }else{ $OP =$OpenPO[$BArray[$i]]; }
										...
										'Days' => '1' , 'Price' => $Price[$BArray[$i]], 'SICCode' => $SICCode[$BArray[$i]],'PurchaseOrderNo' => $PurchaseOrderNo[$BArray[$i]], 'OpenPO' => $OP, 
										'TotalAmount' => $TotalHidden[$BArray[$i]] , 'UpdatedBy'=>$this->session->userdata['userId']);  
										...
										$this->Common_model->update("tbl_booking1",$BookingInfo, $cond1);    
											...
											$SiteLogInfo = array('TableName'=>'tbl_booking1' ,'PrimaryID'=>$BArray[$i], 
											'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
											'SitePage'=>'Edit Booking Request  ','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
											'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
											$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
											...
											for($k=0;$k<count($BookingDate);$k++){ 	
												...
												if($insDate){
													...
													if($Price[$BArray[$i]]!='' && $Price[$BArray[$i]] !=0 ){ 
														if($k==0 ){ 
														$ProductInfo = array('OpportunityID'=>$OpportunityID, 'MaterialID'=>$DescriptionofMaterial[$BArray[$i]], 'SICCode'=>$SICCode[$BArray[$i]],  'BookingDateID'=>$insDate ,'BookingID'=>$BArray[$i] ,
														'DateRequired'=>$BookingDate[$k] ,'UnitPrice'=>$Price[$BArray[$i]] ,'LorryType' =>$LorryType[$BArray[$i]],'PurchaseOrderNo'=>$PurchaseOrderNo[$BArray[$i]], 'Qty'=> $Loads[$BArray[$i]] , 
														'CreateUserID'=>$this->session->userdata['userId'],'EditUserID'=>$this->session->userdata['userId']);         
														$PID = $this->Common_model->insert("tbl_product", $ProductInfo);	
														...
														$SiteLogInfo = array('TableName'=>'tbl_product' ,'PrimaryID'=>$PID, 
														'UpdatedValue'=>$LoadInfoJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
														'SitePage'=>'Edit Booking Request Haulage','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
														'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
														$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
														/* ========================================================================= */
						    ...
						    for($i = 0; $i < count($BArray); $i++ ){     
								...
								if($TotalLoadAllocated[$BArray[$i]]==0 ){	 
									if($BookingType[$BArray[$i]]!="" && $DescriptionofMaterial[$BArray[$i]]!=""  && $BookingDateTime[$BArray[$i]]!="" ){	
										...
										$BookingInfo = array(  'BookingType' => $BookingType[$BArray[$i]] , 'MaterialID' => $DescriptionofMaterial[$BArray[$i]], 
										'TotalTon' => $TotalTon[$BArray[$i]],'TonPerLoad' => $TonPerLoad[$BArray[$i]], 'LoadType' => $LoadType[$BArray[$i]], 
										'LorryType' => $LorryType[$BArray[$i]] , 'MaterialName'=> $MaterialName[$BArray[$i]], 'Loads' => $Loads[$BArray[$i]], 
										'Days' => '1' , 'Price' => $Price[$BArray[$i]], 'SICCode' => $SICCode[$BArray[$i]],'PurchaseOrderNo' => $PurchaseOrderNo[$BArray[$i]], 'OpenPO' => $OP ,  
										'TotalAmount' => $TotalHidden[$BArray[$i]] , 'UpdatedBy'=>$this->session->userdata['userId']);  
										$cond1 = array( 'BookingID' => $BArray[$i] );  
										$this->Common_model->update("tbl_booking1",$BookingInfo, $cond1);    
										...
										//if($BookingID){
											...
											//echo $Loads[$BArray[$i]];
												...
												if($insDate){
													...
													if($Price[$BArray[$i]]!='' && $Price[$BArray[$i]] !=0 ){ 
														if($k==0 ){ 
														$ProductInfo = array('OpportunityID'=>$OpportunityID, 'MaterialID'=>$DescriptionofMaterial[$BArray[$i]], 'SICCode'=>$SICCode[$BArray[$i]],  'BookingDateID'=>$insDate ,'PriceType'=>'1' , 'BookingID'=>$BArray[$i] ,
														'DateRequired'=>$BookingDate[$k] ,'UnitPrice'=>$Price[$BArray[$i]] ,'LorryType' =>$LorryType[$BArray[$i]],'PurchaseOrderNo'=> $PurchaseOrderNo[$BArray[$i]] , 'Qty'=> $Loads[$BArray[$i]] , 'TotalTon' =>$TotalTon[$BArray[$i]],
														'CreateUserID'=>$this->session->userdata['userId'],'EditUserID'=>$this->session->userdata['userId']);         
														$PID = $this->Common_model->insert("tbl_product", $ProductInfo);	
	...
	function ApproveBookingRequest(){
        if($this->isEdit == 0){
            ...
            $result = $this->Common_model->update("tbl_booking_date1",$BookingInfo, $con); 
				...
				$SiteLogInfo = array('TableName'=>'tbl_booking_date1' ,'PrimaryID'=>$BookingDateID, 
				'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
				'SitePage'=>'Approve Booking Request  ','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
				'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
				$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
				/* ========================================================================= */
    ...
    function AllocateBookingRequestAJAX(){
        if($this->isEdit == 0){
			...
			$Driver = $this->Booking_model->getLorryNoDetails($DriverID); 
			$BTText = "";
			if($Booking->BookingType==1){  $BTText = "Collection";  }
			if($Booking->BookingType==2){  $BTText = "Delivery";  }
			if($Booking->BookingType==3){  $BTText = "DayWork";  }
			if($Booking->BookingType==4){  $BTText = "Haulage";  }
			...
			for($i=1;$i<=$Qty;$i++){
				...
				if($TotalLoads != $AllocatedLoads){
					...
					if($Booking->BookingType==2){
						if($TipID==1){
							...
							//if($Driver->AppUser==0){
								...
								'OpportunityID'=> $Booking->OpportunityID, 'CompanyID'=>$Booking->CompanyID ,'DriverName'=>$Driver->DriverName, 
								'RegNumber'=>$Driver->RegNumber ,'Hulller'=>$Driver->Haulier ,'Tare'=>$Driver->Tare ,'driversignature'=>$Driver->ltsignature , 'driver_id'=>$DriverID, 
								'MaterialID'=>$Booking->MaterialID ,'SicCode'=>$Booking->LoadSICCODE,  
								...
								if($TicketID){
									...
									$SiteLogInfo = array('TableName'=>'tbl_tickets' ,'PrimaryID'=>$TicketID, 
									'UpdatedValue'=>$LoadInfoJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
									'SitePage'=>'Allocate '.$BTText.' Booking Harefield ','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
									'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
									$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
								}	
						...
						if($TipID==1){
							if($Driver->AppUser==1){
								...
								'OpportunityID'=> $Booking->OpportunityID, 'CompanyID'=>$Booking->CompanyID ,'DriverName'=>$Driver->DriverName, 
								'RegNumber'=>$Driver->RegNumber ,'Hulller'=>$Driver->Haulier ,'Tare'=>$Driver->Tare ,'driversignature'=>$Driver->ltsignature , 'driver_id'=>$DriverID, 
								'MaterialID'=>$Booking->MaterialID ,'SicCode'=>$Booking->LoadSICCODE, 'is_tml'=>1 , 
								'CreateDate'=>date('Y-m-d H:i:s'),'TypeOfTicket'=>'In', 'IsInBound'=>1 ); 
								$TicketID = $this->Common_model->insert('tbl_tickets', $ticketsInfo); 					
					...
					'TicketID'=>$TicketID,  'TicketUniqueID'=>$TicketUniqueID, 'DriverID'=>$DriverID ,
					'DriverName'=>$Driver->DriverName, 'VehicleRegNo' =>$RegNumber , 'LoadPrice' =>$Booking->Price, 
					'MaterialID'=>$MaterialID ,'AllocatedDateTime'=>$AllocatedDateTime , 'TipID'=>$TipID , 
					'Status'=> $status ,'AutoCreated'=> 1 , 'CreatedBy'=>$this->session->userdata['userId'] ); 
					...
					if($result1){
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$result1, 
						'UpdatedValue'=>$LoadInfoJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'Allocate  '.$BTText.'  Booking ','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
					}	
					...
					if($TipID==1){
						if($Booking->BookingType==2 ){
							...
							if($Update){	
								...
								$SiteLogInfo = array('TableName'=>'tbl_tickets' ,'PrimaryID'=>$TicketID, 
								'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
								'SitePage'=>'Allocate  '.$BTText.'  Booking Harefield == Update  Conveyance in Ticket ','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
								'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
								$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
							}
	...
	function FetchDeliveryTicketNoAJAX(){
        if($this->isEdit == 0){
			...
			if ($data['FetchTicketNumber']['TicketNo'] != '') { echo(json_encode(array('status'=>TRUE,'TicketDate'=>$data['FetchTicketNumber']['TicketDate'],'TicketNo'=>$data['FetchTicketNumber']['TicketNo'],'GrossWeight'=>$data['FetchTicketNumber']['GrossWeight'],'TicketNumber'=>$data['FetchTicketNumber']['TicketNumber'] ,'pdf'=>$pdf ))); }
            else { echo(json_encode(array('status'=>FALSE))); }
	...
	function BookingRequestStageLeftSite($LoadID){
        if($this->isEdit == 0){
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
				...
				'Signature'=>$Signature,'CustomerName'=>$CustomerName, 'Status' => 3 , 'PortalUpdate' => 1 );
				$cond = array( 'LoadID ' => $LoadID  );  
				$update = $this->Common_model->update("tbl_booking_loads1", $LoadInfo, $cond);
				if($update){
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
				...
				if($update){ 	
				    ...
				    /* =================== Site Logs ===================  */
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$LoadID, 
						'FileName'=> $NewFileName , 'UpdatedByUserID'=>$this->session->userdata['userId'],
				'SitePage'=>'conveyance date update pdf ','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
				'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
					/* ===================================== */
	...
	function DateUpdateDelivery($LoadID){
        if($this->isEdit == 0){
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
				...
				$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$LoadID, 
				'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
				'SitePage'=>'delivery date update','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
				'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
				$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
				/* ===================================== */
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST'){  
				...
				if($this->form_validation->run()){ 
				 ...
				 //exit;
					...
					'Street1'=>$Street1,'Street2'=>$Street2,'Town'=>$Town ,'County'=>$County ,'PostCode'=>$PostCode,
					'ContactID '=>$ContactID  , 'ContactName'=>$ContactName , 'ContactMobile'=>$ContactMobile, 'ContactEmail'=>$ContactEmail , 
					'WaitingCharge'=>$WaitingCharge , 'JourneyTime'=>$JourneyTime ,'StartTime'=>$StartTime , 'FinishTime'=>$FinishTime , 
					'WaitingTime'=>$WaitingTime ,  'PriceBy'=>$PriceBy ,  
					'Notes'=>$Notes , 'SubTotalAmount'=>$SubTotalAmount , 'VatAmount'=>$VatAmount , 'TotalAmount'=>$TotalAmount , 'PaymentType'=>$PaymentType , 
					'PaymentRefNo'=>$PaymentRefNo , 'BookedBy'=>$this->session->userdata['userId'],'Status'=> 1 ); 
					$BookingRequestID  = $this->Common_model->insert("tbl_booking_request",$BookingInfo); 
					if($BookingRequestID ){  
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_request' ,'PrimaryID'=>$BookingRequestID, 
						'UpdatedValue'=>$LoadInfoJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'Add Booking Request DayWork','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
						/* ========================================================================= */
						...
						for($i = 0; $i < count($BookingType); $i++ ){  
							if($BookingType[$i]!="" && $DescriptionofMaterial[$i]!=""  && $BookingDateTime[$i]!="" ){	
								...
								$BookingInfo = array('BookingRequestID' => $BookingRequestID , 'BookingType' => '3', 'MaterialID' => $DescriptionofMaterial[$i], 
								'DayWorkType' => $DayWorkType[$i], 'LoadType' => $LoadType[$i], 'LorryType' => $LorryType[$i] , 'MaterialName'=> $MaterialName[$i], 'Loads' => $Loads[$i], 
								'Days' => '1' , 'Price' => $Price[$i], 'SICCode' => $SICCode[$i], 'PurchaseOrderNo' => $PurchaseOrderNo[$i],'OpenPO' => $OP,
								'TotalAmount' => $TotalHidden[$i] , 'BookedBy'=>$this->session->userdata['userId']); 
								$BookingID = '';
								$BookingID = $this->Common_model->insert("tbl_booking1", $BookingInfo);	   
				...
				if($this->form_validation->run()){ 
				 ...
				 //exit;
					...
					$isUpdate = $this->Common_model->update("tbl_booking_request",$BookingInfo, $cond);
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_request' ,'PrimaryID'=>$BookingRequestID, 
						'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'Edit Booking Request DayWork','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
						/* ========================================================================= */
						    ...
						    for($i = 0; $i < count($BArray); $i++ ){     
								...
								if($TotalLoadAllocated[$BArray[$i]]==0 ){	 
									if($BookingType[$BArray[$i]]!="" && $DescriptionofMaterial[$BArray[$i]]!=""  && $BookingDateTime[$BArray[$i]]!="" ){
										...
										$BookingInfo = array(  'BookingType' => '3' , 'MaterialID' => $DescriptionofMaterial[$BArray[$i]], 
										'DayWorkType' => $DayWorkType[$BArray[$i]], 'LoadType' => $LoadType[$BArray[$i]], 'LorryType' => $LorryType[$BArray[$i]] , 
										'MaterialName'=> $MaterialName[$BArray[$i]], 'Loads' => $Loads[$BArray[$i]], 
										'Days' => '1' , 'Price' => $Price[$BArray[$i]], 'SICCode' => $SICCode[$BArray[$i]],  'PurchaseOrderNo' => $PurchaseOrderNo[$BArray[$i]],'OpenPO' => $OP,
										'TotalAmount' => $TotalHidden[$BArray[$i]] , 'UpdatedBy'=>$this->session->userdata['userId']);  
										$cond1 = array( 'BookingID' => $BArray[$i] );  
										$this->Common_model->update("tbl_booking1",$BookingInfo, $cond1);    
				...
				if($this->form_validation->run()){ 
				 ...
				 //exit;
					...
					if($BookingRequestID ){  
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_request' ,'PrimaryID'=>$BookingRequestID, 
						'UpdatedValue'=>$LoadInfoJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'Add Booking Request Haulage','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
						/* ========================================================================= */
						...
						for($i = 0; $i < count($BookingType); $i++ ){  
							if($BookingType[$i]!="" && $DescriptionofMaterial[$i]!=""  && $BookingDateTime[$i]!="" ){		 
								...
								$BookingInfo = array('BookingRequestID' => $BookingRequestID , 'TipID'=>$TipID,'BookingType' => '4' , 'MaterialID' => $DescriptionofMaterial[$i], 
								'LoadType' => $LoadType[$i], 'LorryType' => $LorryType[$i] , 'MaterialName'=> $MaterialName[$i], 'Loads' => $Loads[$i], 
								'Days' => '1' , 'Price' => $Price[$i], 'SICCode' => $SICCode[$i],  'PurchaseOrderNo' => $PurchaseOrderNo[$i],'OpenPO' => $OP,
								'TotalAmount' => $TotalHidden[$i] , 'BookedBy'=>$this->session->userdata['userId']); 
								$BookingID = '';
								...
								if($BookingID){ 
									...
									for($k=0;$k<count($BookingDate);$k++){ 	
										...
										if($insDate){
											...
											if($Price[$i]!='' && $Price[$i] !=0 ){ 
											if($k==0 ){ 
												$ProductInfo = array('OpportunityID'=>$OpportunityID, 'MaterialID'=>$DescriptionofMaterial[$i],
												'TipID'=>$TipID, 'TipName'=>$TipName[$i], 
												'SICCode'=>$SICCode[$i],  'BookingDateID'=>$insDate ,'BookingID'=>$BookingID ,
												'DateRequired'=>$BookingDate[$k] ,'UnitPrice'=>$Price[$i] ,'LorryType' => $LorryType[$i] ,
												'PurchaseOrderNo'=>$PurchaseOrderNo[$i] , 'Qty'=> $Loads[$i] , 'PriceType'=>'0' ,
												'CreateUserID'=>$this->session->userdata['userId'],'EditUserID'=>$this->session->userdata['userId']);         
												$PID = $this->Common_model->insert("tbl_product", $ProductInfo);	
						    ...
						    for($i = 0; $i < count($BArray); $i++ ){     
								...
								if($TotalLoadAllocated[$BArray[$i]]==0 ){	 
									if($BookingType[$BArray[$i]]!="" && $DescriptionofMaterial[$BArray[$i]]!=""  && $BookingDateTime[$BArray[$i]]!="" ){
										...
										$BookingInfo = array( 'LoadType' => $LoadType[$BArray[$i]], 'LorryType' => $LorryType[$BArray[$i]] , 'Loads' => $Loads[$BArray[$i]], 
										'Days' => '1' , 'Price' => $Price[$BArray[$i]], 'SICCode' => $SICCode[$BArray[$i]],'PurchaseOrderNo' => $PurchaseOrderNo[$BArray[$i]], 
	...
	function BookingRequestDayworkFinish($LoadID){
        if($this->isEdit == 0){
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
				...
				$LoadInfo = array('SiteInDateTime'=>$SiteInDateTime,'SiteOutDateTime'=>$SiteOutDateTime,'JobEndDateTime'=>$JobEndDateTime,'Notes2'=>$Notes2, 'ReceiptName'=>$TicketUniqueID.".pdf",
				'Signature'=>$Signature,'CustomerName'=>$CustomerName, 'Status' => 4 );
				$cond = array( 'LoadID ' => $LoadID  );  
				$update = $this->Common_model->update("tbl_booking_loads1", $LoadInfo, $cond);
				if($update){
		...
		//	echo count($data['SplitExcelDayWorkTicketsAll']);
			...
			for($j=0;$j<count($Array);$j++){ 
				...
				$cname = "";$oname = "";
					...
					if(is_numeric($data['SplitExcelDayWorkTicketsAll'][$i]['Price'])) { $Price = $data['SplitExcelDayWorkTicketsAll'][$i]['Price']; }else{ $Price = 0; } 		
					//$TPrice = $TPrice + $Price;
					$TPrice = floor(($TPrice + $Price)*100)/100;
	...
	function DateUpdateHaulage($LoadID){
        if($this->isEdit == 0){
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
				...
				$LoadInfo = array('JobStartDatetime'=>$JobStartDatetime , 
				'SiteInDatetime'=>$SiteInDatetime , 'SiteOutDatetime'=>$SiteOutDatetime , 
				'SiteInDatetime2'=>$SiteInDatetime2 , 'SiteOutDatetime2'=>$SiteOutDatetime2 , 
		...
		//	echo count($data['SplitExcelDayWorkTicketsAll']);
			...
			for($j=0;$j<count($Array);$j++){ 
				...
				$cname = "";$oname = "";
					...
					if(is_numeric($data['SplitExcelHaulageTicketsAll'][$i]['LoadPrice'])) { $Price = $data['SplitExcelHaulageTicketsAll'][$i]['LoadPrice']; }else{ $Price = 0; } 		
					 //$TPrice = $TPrice + $Price;
					  $TPrice = floor(($TPrice + $Price)*100)/100;
					$object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, $Price );      
 ...
 //$zipFile = base_url("SplitXLS/".$zname); 
	...
	function BookingRequestHaulageFinish($LoadID){
        if($this->isEdit == 0){
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
				if($data['LoadInfo']->Status==4 || $data['LoadInfo']->Status==5 || $data['LoadInfo']->Status==6 ){ redirect('RequestLoads');  }
				...
				*/	
				...
				$LoadInfo = array('SiteInDateTime'=>$SiteInDateTime,'SiteOutDateTime'=>$SiteOutDateTime,
				'SiteInDateTime2'=>$SiteInDateTime2,'SiteOutDateTime2'=>$SiteOutDateTime2,
				'JobEndDateTime'=>$JobEndDateTime,'Notes2'=>$Notes2, 'ReceiptName'=>$TicketUniqueID.".pdf",
				'CustomerName'=>$CustomerName, 
				'CustomerName2'=>$CustomerName2, 'Status' => 4 );
				$cond = array( 'LoadID ' => $LoadID  );  
				$update = $this->Common_model->update("tbl_booking_loads1", $LoadInfo, $cond);
	...
	function FetchBookingRequestAJAX(){
        if($this->isEdit == 0){
			...
			if($BookingInfo[0]->BookingType==1){ $BookingType =  "Collection"; } 
			if($BookingInfo[0]->BookingType==2){ $BookingType =  "Delivery"; }  
			if($BookingInfo[0]->BookingType==3){ $BookingType =  "DayWork"; }   
			if($BookingInfo[0]->BookingType==4){ $BookingType =  "Haulage"; }   
        ...
        if($this->isEdit == 0){
			...
			if($UpdateLoadInfo) {   
				if($BookingType ==1 ){
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$LoadID, 
						'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'booking update conveyance','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
						///* ===================================== 
						...
						///* =================== Site Logs ===================   
							...
							$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$LoadID, 
							'FileName'=> $NewFileName , 'UpdatedByUserID'=>$this->session->userdata['userId'],
							'SitePage'=>'booking update conveyance pdf','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
							'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
							$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
						// =====================================  
						...
						if($TicketID>0){	 
							...
							$SiteLogInfo = array('TableName'=>'tbl_tickets' ,'PrimaryID'=>$TicketID, 
							'UpdatedValue'=>$TInfoJson." => ".$CondJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
							'SitePage'=>'booking update ticket ','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
							'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
							$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
							/* ===================================== */
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$LoadID, 
						'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'booking update delivery','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
						///* ===================================== 
                    ...
                    exit;
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$LoadID, 
						'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'booking update daywork','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
						///* ===================================== 
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_loads1' ,'PrimaryID'=>$LoadID, 
						'UpdatedValue'=>$LoadInfoJson." => ".$condJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'booking update Haulage','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);  
						///* ===================================== 
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST'){  
				...
				if($this->form_validation->run()){ 
				 ...
				 //exit;
					...
					$BookingInfo = array( 'CompanyID'=>$CompanyID , 'CompanyName'=>$CompanyName , 'OpportunityID'=>$OpportunityID, 'OpportunityName'=>$OpportunityName , 
					'Street1'=>$Street1,'Street2'=>$Street2,'Town'=>$Town ,'County'=>$County ,'PostCode'=>$PostCode,
					'ContactID '=>$ContactID  , 'ContactName'=>$ContactName , 'ContactMobile'=>$ContactMobile, 'ContactEmail'=>$ContactEmail , 
					'WaitingCharge'=>$WaitingCharge , 'WaitingTime'=>$WaitingTime , 'PriceBy'=>$PriceBy ,  
					'Notes'=>$Notes , 'SubTotalAmount'=>$SubTotalAmount , 'VatAmount'=>$VatAmount , 'TotalAmount'=>$TotalAmount , 'PaymentType'=>$PaymentType , 
					'PaymentRefNo'=>$PaymentRefNo , 'BookedBy'=>$this->session->userdata['userId'],'Status'=> 1,'DuplicateBooking'=> 1 ); 
					$BookingRequestID  = $this->Common_model->insert("tbl_booking_request",$BookingInfo); 
					if($BookingRequestID ){  
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_request' ,'PrimaryID'=>$BookingRequestID, 
						'UpdatedValue'=>$LoadInfoJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'Duplicate Booking Request','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
						/* ========================================================================= */
						...
						for($i = 0; $i < count($BookingType); $i++ ){   
							if($BookingType[$i]!="" && $DescriptionofMaterial[$i]!=""  && $BookingDateTime[$i]!="" ){	 
								$BookingInfo = array('BookingRequestID' => $BookingRequestID , 'BookingType' => $BookingType[$i] , 'MaterialID' => $DescriptionofMaterial[$i], 
								'LoadType' => $LoadType[$i], 'LorryType' => $LorryType[$i] , 'MaterialName'=> $MaterialName[$i], 'Loads' => $Loads[$i], 
								'Days' => '1' , 'Price' => $Price[$i], 'SICCode' => $SICCode[$i],'PurchaseOrderNo' => $PurchaseOrderNo[$i], 'OpenPO' => $OpenPO[$i], 
								'TotalAmount' => $TotalHidden[$i] , 'BookedBy'=>$this->session->userdata['userId']); 
								$BookingID = '';
			...
			if ($this->input->server('REQUEST_METHOD') === 'POST'){  
				...
				// exit;
					...
					if($BookingRequestID ){  
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_request' ,'PrimaryID'=>$BookingRequestID, 
						'UpdatedValue'=>$LoadInfoJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'Duplicate DayWork Booking Request','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
						/* ========================================================================= */
						//echo "<PRE>"; 
						...
						for($i = 0; $i < count($BookingType); $i++ ){  
							if($BookingType[$i]!="" && $DescriptionofMaterial[$i]!=""  && $BookingDateTime[$i]!="" ){		 
								$BookingInfo = array('BookingRequestID' => $BookingRequestID , 'BookingType' => '3', 'MaterialID' => $DescriptionofMaterial[$i], 
								'DayWorkType' => $DayWorkType[$i], 'LoadType' => $LoadType[$i], 'LorryType' => $LorryType[$i] , 'MaterialName'=> $MaterialName[$i], 'Loads' => $Loads[$i], 
								'Days' => '1' , 'Price' => $Price[$i], 'SICCode' => $SICCode[$i], 'PurchaseOrderNo' => $PurchaseOrderNo[$i],'OpenPO' => $OpenPO[$i],'TotalAmount' => $TotalHidden[$i] , 'BookedBy'=>$this->session->userdata['userId']); 
								$BookingID = '';
								$BookingID = $this->Common_model->insert("tbl_booking1", $BookingInfo);	   
								if($BookingID){ 
				...
				if($this->form_validation->run()){ 
				 ...
				 //exit;
					...
					if($BookingRequestID ){  
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_request' ,'PrimaryID'=>$BookingRequestID, 
						'UpdatedValue'=>$LoadInfoJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'Duplicate Booking Request Haulage','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
						/* ========================================================================= */
				...
				if($this->form_validation->run()){ 
				 ...
				 //exit;
					...
					if($BookingRequestID ){  
						...
						$SiteLogInfo = array('TableName'=>'tbl_booking_request' ,'PrimaryID'=>$BookingRequestID, 
						'UpdatedValue'=>$LoadInfoJson, 'UpdatedByUserID'=>$this->session->userdata['userId'],
						'SitePage'=>'Duplicate Booking Request Tonnage','RemoteIPAddress'=>$_SERVER['REMOTE_ADDR'],'BrowserAgent'=>getBrowserAgent(),
						'AgentString'=>$this->agent->agent_string(),'UserPlatform'=>$this->agent->platform());          
						$this->Common_model->insert("tbl_site_logs", $SiteLogInfo);   
						/* ========================================================================= */
