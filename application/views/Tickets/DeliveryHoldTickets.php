<div class="content-wrapper"> 
    <section class="content-header"> <h1> <i class="fa fa-users"></i> Delivery PDA HOLD  Tickets Management    </h1>    </section> 
            ...
            <div class="col-xs-12">
				<div class="box">
					<div class="box-header">
						<h3 class="box-title"><b>Delivery PDA HOLD  Tickets List</b></h3>
						...
						<?php if($CountHoldTicketsPDF[0]['ccnt']>0){ ?>
						<a class="btn btn-danger" href="<?php echo base_url(); ?>DeliveryHoldTicketsPDF" target="_blank" style="float:right;margin: 6px "><i class="fa fa-plus"></i> Create PDFs (<?php echo $CountHoldTicketsPDF[0]['ccnt']; ?>)</a>
						<?php } ?>
					</div> 
					<div class="box-body">
...
var GetTableMetaApiEndpoint = 'DeliveryHoldTicketsTableMeta';//Endpoint returning Table Metadata 
var GetTableDataApiEndpoint = 'AJAXDeliveryHoldTickets';//Endpoint processing and return Table Data
	...
	function initializeDataTable() {
		...
		table = $('#dtexample').DataTable({
			...
			createdRow: function (row, data, dataIndex) {  
				var DName = "";
				if(data["LoadID"]!=0){ DName = data["LoginDriverName"]; }else{ DName = data["DriverName"]; } 
				...
				$(row).find("td:eq(-1)").html(' <a class="btn btn-sm btn-info" href="'+baseURL+'edit-'+data["TypeOfTicket"]+'-ticket/'+data["TicketNo"]+'" title="Edit Ticket"><i class="fa fa-pencil"></i></a>');
				//$(row).find("td:eq(-1)").html(' <a class="btn btn-sm btn-danger deleteTicket" href="#" data-ticketno="'+data["TicketNo"]+'" title="Delete"><i class="fa fa-trash"></i></a>');
				...
				$(row).find("td:eq(4)").html(data["MaterialName"]);	
				},  
	...
	$(document).ready(function() {
		...
		jQuery(document).on("change", ".GridGross", function(){ 
			...
			var TicketNo = $(this).attr("data-TicketNo"),    
				...
				Gross = $(this).val(),  
				hitURLUP = baseURL + "AJAXUpdateHoldGrossWeight",
				currentRow = $(this);	    
				//alert(BookingDateID);
		...
		jQuery(document).on("change", ".GridDate", function(){ 
			...
			var TicketNo = $(this).attr("data-TicketNo"),     
				TicketDate = $(this).val(),  
				hitURLUP1 = baseURL + "AJAXUpdateHoldTicketDate",
				currentRow = $(this);	    
				//alert(BookingDateID);
				//Price = $('#Price'+BookingDateID).val(),  
				jQuery.ajax({
