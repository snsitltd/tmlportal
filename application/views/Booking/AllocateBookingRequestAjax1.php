...
<div class="row">  
	...
	<div class="col-md-4">
		<div id="d<?php echo $BookingDateID; ?>">
			...
			if(!empty($LorryRecords)){ foreach ($LorryRecords as $rl){  
			
			if(!empty($rl->next_wordorder_date) && $rl->next_wordorder_date !== "0000-00-00"){
				$bookingDate = strtotime($BookingInfo['BookingDate']);
				$workOrderDate = strtotime($rl->next_wordorder_date);
				if ($bookingDate >= $workOrderDate){
					continue;
				}
			}
