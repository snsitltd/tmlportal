...
<form id="CreateInvoice" name="CreateInvoice"  action="<?php echo base_url('BookingCreateInvoiceTonnage/'.$RequestInfo['BookingRequestID']); ?>"  method="post" role="form" > 		
...
<input type="hidden" value="20"  name="TaxRate" id="TaxRate"  >  		
  ...
  <thead>
	<tr>  
		...
		<th width="30" >Total Tons </th> 
		<th width="100" >Price/Ton</th>     
	...
	<tr class="<?php echo $cls1; ?>" data-BookingDateID="<?php echo $record->BookingDateID; ?>" >
		...
		<td align="right" ><?php echo (int)$record->TotalTon ?>
			 <input type="hidden" value="<?php echo (int)$record->TotalTon  ?>" name="TotalTon<?php echo $record->BookingDateID; ?>" id="TotalTon<?php echo $record->BookingDateID; ?>" > 
		</td>
		<td align="right" >
		...
		<td colspan="10" > 			 
			...
			for($i=0;$i<count($LD);$i++){  
			 ...
			 <tr class="<?php echo $cls1; ?>">  
				...
				<td><input type="text" value="<?php echo $LD[$i]['Price']; ?>" class="LoadPrice<?php echo $record->BookingDateID; ?>" data-BookingDateID="<?php echo $record->BookingDateID; ?>" data-LoadID="<?php echo $LD[$i]['LoadID']; ?>" style="width:60px" name="LoadPrice<?php echo $LD[$i]['LoadID']; ?>" > 
				...
				<input type="hidden" value="<?php echo $LD[$i]['Net']; ?>"  name="Net[]" id="Net<?php echo $LD[$i]['LoadID']; ?>" >
				
				</td>
				<td><span class="TotalLoadPrice<?php echo $LD[$i]['BookingDateID']; ?>" id="TLP<?php echo $LD[$i]['LoadID']; ?>"><?php echo (float)round(($LD[$i]['Net']/1000)*$LD[$i]['Price'],2); ?></span> 
				<input type="hidden" value="<?php echo (float)round(($LD[$i]['Net']/1000)*$LD[$i]['Price'],2); ?>"  name="TotalLoadPrice<?php echo $LD[$i]['LoadID']; ?>" id="TotalLoadPrice<?php echo $LD[$i]['LoadID']; ?>" >
				<input type="hidden" value="<?php echo (float)round(($LD[$i]['Net']/1000),2); ?>"  name="TotalLoadTon<?php echo $LD[$i]['LoadID']; ?>" id="TotalLoadTon<?php echo $LD[$i]['LoadID']; ?>" ></td>
			</tr>   
			<?php } ?>
			</tbody>	
		</table> 
...
$(document).ready(function() {
	...
	jQuery(document).on("click", ".PriceUpdate", function(){   
		var BookingDateID = $(this).attr("data-BookingDateID"), 
			TotalTon = $('#TotalTon'+BookingDateID).val(),
			Price = $('#Price'+BookingDateID).val();  
			var s1=0; 
			$('.TotalLoadPrice'+BookingDateID).each(function(){
				s1 += parseFloat($(this).html());
				//alert($(this).html())
			});
			
			$('#TP'+BookingDateID).html(parseFloat(s1).toFixed(2));
			
			$('#TotalPrice'+BookingDateID).val(parseFloat(s1).toFixed(2));
	...
	jQuery(document).on("keyup","[class^='LoadPrice']", function(){   
		//alert("sdfsdfsdf");
		var BookingDateID = $(this).attr("data-BookingDateID"), 
			LoadID = $(this).attr("data-LoadID"), 
			Price = parseFloat($( this ).val()); 
			
			$('#TLP'+LoadID).html(parseFloat($(this).val()*( $('#Net'+LoadID).val()/1000 )));  
			var sum = 0; 
			//$('.LoadPrice'+BookingDateID).each(function(){  
			//	sum += parseFloat($(this).val());
			//});  
			...
			//alert(sum)
			$('#TP'+BookingDateID).html(parseFloat(sum).toFixed(2));
			//$('#TLP'+LoadID).html(parseFloat(sum).toFixed(2));
