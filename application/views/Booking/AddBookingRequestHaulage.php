		...
		//$('.BookingDateTime , .LoadType , .Loads , .BookingType').datepicker().on('changeDate click', function (ev) { 
			...
			if(OpportunityID!='' && OpportunityID!=0 && MaterialID!='' && BookingType!='' ){ 
				...
				if(BookingDateTime!=""){
					...
					var HitUrl = baseURL + "ShowHaulageOppoProductPriceAJAX"; 
					$.ajax({
						type : "POST",
						dataType : "json",
						url : HitUrl,
						data : { 'OpportunityID' : OpportunityID,'TipName' : TipName,'DateRequired' : DateRequired,'LorryType' : LorryType} 
