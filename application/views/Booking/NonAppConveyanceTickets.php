<div class="content-wrapper"> 
    <section class="content-header"> <h1> <i class="fa fa-users"></i>NonApp Collection Tickets </h1>    </section> 
			...
			<div class="col-md-12">
				<div class="nav-tabs-custom">
					<ul class="nav nav-tabs">
						...
						<li class=""><a href="<?php echo base_url('NonAppDeliveryTickets'); ?>"  aria-expanded="false">Delivery</a></li>      
					</ul> 
...
var GetTableMetaApiEndpoint = 'NonAppConveyanceTicketsTableMeta';//Endpoint returning Table Metadata 
var GetTableDataApiEndpoint = 'AjaxNonAppConveyanceTickets';//Endpoint processing and return Table Data
	...
	function initializeDataTable() {
		...
		table = $('#dtexample').DataTable({
			...
			createdRow: function (row, data, dataIndex) {  
				...
				$(row).find("td:eq(5)").html('<button  class="btn btn-primary  btn-info TipUpdate" data-TipID = "'+data["TipID"]+'" data-LoadID = "'+data["LoadID"]+'" title="Click To Update Tip Address">'+Tip+'</button> ');
				$(row).find("td:eq(6)").html('<button  class="btn btn-warning  btn-info MaterialUpdate" data-MaterialID = "'+data["MaterialID"]+'" data-LoadID = "'+data["LoadID"]+'" title="Click To Update Material ">'+data["MaterialName"]+'</button> ');	  
				$(row).find("td:eq(7)").html(dname);  
				$(row).find("td:eq(8)").html(vreg);      
				$(row).find("td:eq(9)").html('<input type="text" class="GrossUpdate" data-Tare="'+data["Tare"]+'" data-LoadID="'+data["LoadID"]+'"  id="GrossWeight'+data["LoadID"]+'" style="text-align:right;width:50px" value="'+data["GrossWeight"]+'" name="GrossWeight'+data["LoadID"]+'"   maxlength="20"  >');	
				$(row).find("td:eq(11)").html('<span id="Net'+data["LoadID"]+'">'+data["Net"]+'</span>');    
