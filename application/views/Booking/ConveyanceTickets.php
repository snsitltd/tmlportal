...
<div class="content-wrapper">
	<section class="content-header">
		<h1> <i class="fa fa-users"></i>Collection / Delivery Tickets
			<a href="https://tml.snsitltd.com/cron/CronDocuments.php" target="_blank" class="btn btn-info" name="exportxls" id="exportxls" style="float:right;margin: 6px "> Update HandWritten Tickets</a>
		</h1>
	</section>
		...
		table = $('#dtexample').DataTable({
			...
			createdRow: function(row, data, dataIndex) {
				...
				if (data["Status"] == "Finished") {
					Status = '<button  class="btn   btn-success StatusUpdate" data-PDF = "' + data["ReceiptName"] + '" data-Status = "' + data["Status"] + '" data-LoadID = "' + data["LoadID"] + '" title="Click To Update Status ">' + data["Status"] + '</button> ';
				}
				...
				if (data["Status"] == "Wasted") {
					Status = '<button  class="btn   btn-warning StatusUpdate" data-PDF = "' + data["ReceiptName"] + '"  data-Status = "' + data["Status"] + '" data-LoadID = "' + data["LoadID"] + '" title="Click To Update Status ">' + data["Status"] + '</button> ';
				}
