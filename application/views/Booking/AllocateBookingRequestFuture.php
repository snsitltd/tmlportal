			...
			<div class="col-md-12">
				<div class="nav-tabs-custom"> 
					<ul class="nav nav-tabs">
						<li class=""><a href="<?php echo base_url('AllocateBookingRequest'); ?>" aria-expanded="true">Today's </a></li> 
						<li class=""><a  href="<?php echo base_url('AllocateBookingRequestOverdue'); ?>"  aria-expanded="false">OverDue </a></li>
						<li class="active"><a href="#Future" data-toggle="tab" aria-expanded="false">Future </a></li> 
						<li class=""><a href="<?php echo base_url("AllocateBookingRequestFinished"); ?>"   aria-expanded="false">Finished</a></li>     
	...
	<div id="example2_wrapper" style="min-height:500px" class="dataTables_wrapper form-inline dt-bootstrap table-responsive"><div class="row"><div class="col-sm-6"></div><div class="col-sm-6"></div></div><div class="row"><div class="col-sm-12">                  
		  ...
		  <thead>
			<tr > 
				...
				<th width="30" >Load/Lorry Type</th>     
				<th width="10" >No. Of Loads/Lorry </th>      
				<th width="10" >Pending Allocate</th>   
			...
			<tr  class="<?php echo $cls1; ?>" >
				<td ><a class="ShowLoads" data-BookingDateID="<?php echo $record->BookingDateID; ?>" herf="#" ><?php echo $record->BookingRequestID; ?></a></td>
				<td><?php if($record->BookingType ==1){ echo "Collection"; }if($record->BookingType ==2){ echo "Delivery"; }  if($record->BookingType ==3){ echo "DayWork"; } if($record->BookingType ==4){ echo "Haulage"; }  ?></td>
				...
				<td><?php if($record->TonBook ==1){ echo "Tonnage"; }else{ if($record->LoadType==1){ echo "Loads"; } if($record->LoadType==2){ echo "TurnAround"; } } ?></td>
				...
				<a class="btn btn-sm  btn-info Allocate" herf="#"  data-BookingRequestID="<?php echo $record->BookingRequestID; ?>"  data-BookingNo="<?php echo $record->BookingID; ?>"  data-PendingLoad="<?php echo $pLoads; ?>"  data-LoadType="<?php echo $record->LoadType; ?>"  data-BookingDate="<?php echo $record->BookingDateTime; ?>"  data-BookingDateID="<?php echo $record->BookingDateID; ?>"   title="Allocate Booking"><i class="fa fa-check"></i> </a>
...
//##############################################  Open Allocate Modal Box ##############################################
		...
		jQuery(document).on("click", ".Allocate", function(){  
				var BookingID = $(this).attr("data-BookingNo"), 
					...
					hitURL1 = baseURL + "AllocateBookingRequestAJAX1",
					currentRow = $(this);  
					...
					dataType : "json",
					url : hitURL1,
					data : { 'BookingRequestID' : BookingRequestID, 'BookingID' : BookingID, 'BookingDateID' : BookingDateID ,'LoadType' : LoadType ,'PendingLoad' : PendingLoad ,'BookingDate' : BookingDate  } 
...
//##############################################  Allocate Booking for today and Overdue ##############################################
		...
		jQuery(document).on("click", ".AllocateBooking", function(){  
				var BookingID = $(this).attr("data-BookingNo"),
					...
					hitURL1 = baseURL + "AllocateBookingRequestAJAX",
					currentRow = $(this);    
					if(TipID!="" && TipID>0){ 
					if(Qty!="" && Qty>0){ 
					if(Driver!=undefined){ 
						...
						if(confirmation!=null){ 
							if(confirmation!=""){ 
								...
								dataType : "json",
								url : hitURL1,
								data : { 'BookingRequestID' : BookingRequestID,'BookingID' : BookingID,'BookingDateID' : BookingDateID,'BookingDate' : '','DriverID' :DriverID,'TipID' :TipID,'Qty' :Qty,'AllocatedDateTime' :AllocatedDateTime,'MaterialID' :MaterialID,'Loads' :Loads,'TotalLoads' :TotalLoads,'confirmation' :confirmation } 
								}).success(function(data){ 
									//alert(JSON.stringify( data ));   
									//console.log(data);  
									if(data.status == true) { 
...
/*##############################################  Delete Booking   ##############################################*/		
		...
		//$('#empModal').modal('show');  
				var BookingDateID = $(this).attr("data-BookingDateID"),  
					...
					hitURL1 = baseURL + "ShowLoadsDeleteBookingRequestAJAX",
					currentRow = $(this);  
				//console.log(confirmation); 
				//alert(PendingLoads)
				jQuery.ajax({
...
/*##############################################  Update Booking Qty   ##############################################*/		
		...
		//$('#empModal').modal('show');   
				var BookingID = $(this).attr("data-BookingID"),  
					...
					PendingLoads = $('#loads'+BookingDateID).html(),
					TotalLoads = $('#TotalLoads'+BookingDateID).html(),
					hitURL1 = baseURL + "ShowUpdateBookingRequestAJAX",
					currentRow = $(this);  
				//console.log(confirmation); 
				//alert(PendingLoads)
				jQuery.ajax({
