...
<h1 align="center">Material Tipped IN</h1>
...
<div style="padding-left: 10px; padding-right: 10px;">
  ...
  if(!empty($resultcompany)){
    ...
    foreach ($resultcompany as $key => $value) { ?>

<div class="col-md-12" style="padding: 10px;margin-top: 20px;width: 100%;font-size: 14px;box-shadow: 1px 1px 6px #ddd; background-color:#c2c3c4  "><?php echo $value->CompanyName; ?> </div><br>


  <?php 

  foreach ($value->resultmaterials as $key1 => $value2) { ?>

<div class="col-md-12" style="padding: 10px;margin-top: 20px;width: 100%;font-size: 14px;box-shadow: 1px 1px 6px #ddd; background-color:#ededed  "><?php echo $value2->MaterialName; ?> </div>
<br>
...
<table class="table table-striped" border="1" autosize="1" >
  <thead>
    ...
    <tbody>
        ...
        $tipped = $this->db->query("select ticket.*,Opportunity.OpportunityName from  tbl_tickets as ticket left join tbl_opportunities as Opportunity on Opportunity.OpportunityID = ticket.OpportunityID  where ticket.MaterialID = ".$value2->MaterialID." AND ticket.CompanyID = '".$value->CompanyID."' AND ticket.CreateDate between '".$firstDate."' and '".$SecondDate."'");
