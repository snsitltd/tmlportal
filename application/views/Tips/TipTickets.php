...
<div class="content-wrapper" style="height:800px"> 
    <section class="content-header"> <h1> <i class="fa fa-users"></i> Tip Tickets : <?php echo $TipInfo['TipName'];?> </h1>    </section> 
	...
	function initializeDataTable() {
		...
		table = $('#dtexample').DataTable({
			...
			"columnDefs": [  
				...
				{ "width": "80px", "targets": 8 ,"orderable": false }
				], 
			rowId: 'LoadID', 
	...
	$(document).ready(function() { 
		...
		jQuery(document).on("change", ".TipTicketNoUpdate", function(){  
			var TipTicketID = $(this).attr("data-TipTicketID"),      
				...
				dataType : "json",
				url : hitURL1,
				data : { 'TipTicketID' : TipTicketID, 'TipTicketNo' : TipTicketNo  } 
				}).success(function(data){ 
		...
		var TipTicketID = $('#TipTicketID').val(),   
			...
			if(reservation !="" || TipDateTime !=""){ 
				...
				dataType : "json",
				url : hitURLCon,
				data : { 'Search' : Search,'TipID' : TipID, 'TipTicketID' : TipTicketID,'TipDateTime' : TipDateTime ,'reservation' : reservation ,'ConveyanceNo' : ConveyanceNo ,'SiteAddress' : SiteAddress ,'DriverName' : DriverName ,'MaterialName' : MaterialName ,'TipTicketNo' : TipTicketNo ,'Remarks' : Remarks  } 
				}).success(function(data){ 
