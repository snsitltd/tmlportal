...
class Booking_model extends CI_Model
{
	...
	function GetAllocatedBookingData1()
	{
	...
	public function GetDriverRequestLoadsDelivery($searchdate, $driver)
	{
	...
	public function GetDriverRequestLoadsDayWork($searchdate, $driver)
	{
	...
	public function GetDriverRequestLoadsHaulage($searchdate, $driver)
	{
		...
		//$this->db->where(' DATE_FORMAT(tbl_booking_loads1.AllocatedDateTime,"%Y-%m-%d") ', $firstDate);    
		//$this->db->where(' DATE_FORMAT(tbl_booking_loads1.JobStartDateTime,"%Y-%m-%d") ', $firstDate);    
		$this->db->where(' DATE_FORMAT(tbl_booking_loads1.SiteOutDateTime2,"%Y-%m-%d") ', $firstDate);
	...
	public function GetBookingRequestPriceByPending()
	{
		...
		if (!empty(trim($_POST['sort']))) {
			$Sort = explode(' ', trim($_POST['sort']));
			//print_r($Sort);
			if ($Sort[0] == 'PurchaseOrderNumber') {
				$columnName = 'tbl_booking_request.PurchaseOrderNumber ';
			}
			if ($Sort[0] == 'Price') {
				$columnName = 'tbl_booking1.Price ';
			}
			if ($Sort[0] == 'BookingRequestID') {
				$columnName = 'tbl_booking_request.BookingRequestID ';
			}
			if ($Sort[0] == 'BookingDate') {
				$columnName = ' DATE_FORMAT(tbl_booking_date1.BookingDate,"%Y%m%d ") ';
			}
			if ($Sort[0] == 'CompanyName') {
				$columnName = 'tbl_booking_request.CompanyName ';
			}
			if ($Sort[0] == 'OpportunityName') {
				$columnName = 'tbl_booking_request.OpportunityName ';
			}
			if ($Sort[0] == 'MaterialName') {
				$columnName = 'tbl_booking1.MaterialName ';
			}

			if ($Sort[0] == 'TotalTon') {
				$columnName = 'tbl_booking1.TotalTon ';
			}
			if ($Sort[0] == 'TonPerLoad') {
				$columnName = 'tbl_booking1.TonPerLoad ';
			}

			if ($Sort[0] == 'BookingType') {
				$columnName = 'tbl_booking1.BookingType ';
			}
			if ($Sort[0] == 'LoadType') {
				$columnName = 'tbl_booking1.LoadType ';
			}
			if ($Sort[0] == 'Loads') {
				$columnName = 'tbl_booking1.Loads ';
			}
			if ($Sort[0] == 'LorryType') {
				$columnName = 'tbl_booking1.LorryType ';
			}
			if ($Sort[0] == 'Notes') {
				$columnName = 'tbl_booking_request.Notes ';
			}
			if ($Sort[0] == 'BookedName') {
				$columnName = ' tbl_users.name ';
			}
			if ($Sort[0] == 'CreateDateTime') {
				$columnName = ' DATE_FORMAT(tbl_booking_date1.CreateDateTime,"%Y%m%d %T") ';
			}
	...
	public function SplitExcelDelOppoGroup($CompanyName, $OpportunityName, $Reservation1, $SiteOutDateTime, $TicketNumber, $MaterialName, $DriverName, $VehicleRegNo, $WaitTime, $Status, $Price, $Search, $PurchaseOrderNo)
	{
		...
		if (!empty(trim($_POST['sort']))) {
			...
			if ($Sort[0] == 'VehicleRegNo') {
				$columnName = 'tbl_booking_loads1.VehicleRegNo';
			}
			if ($Sort[0] == 'DriverName') {
				$columnName = ' tbl_drivers.DriverName ';
			}
			if ($Sort[0] == 'Status') {
				$columnName = ' tbl_booking_loads1.Status ';
	...
	public function GetNonAppConveyanceTickets()
	{
		...
		if (!empty(trim($_POST['sort']))) {
			...
			if ($Sort[0] == 'NonAppConveyanceNo') {
				$columnName = 'tbl_booking_loads1.NonAppConveyanceNo ';
			}
			if ($Sort[0] == 'SiteOutDateTime') {
				$columnName = ' DATE_FORMAT(tbl_booking_loads1.SiteOutDateTime,"%Y%m%d%H%i%S") ';
			}
			...
			if ($Sort[0] == 'TipName') {
				$columnName = 'tbl_tipaddress.TipName ';
			}
			if ($Sort[0] == 'DriverName') {
				$columnName = ' tbl_drivers.DriverName ';
			}
			...
			if ($Sort[0] == 'GrossWeight') {
				$columnName = ' tbl_booking_loads1.GrossWeight ';
			}
			if ($Sort[0] == 'Tare') {
				$columnName = ' tbl_booking_loads1.Tare ';
			}
			if ($Sort[0] == 'Net') {
				$columnName = ' tbl_booking_loads1.Net ';
			}
	...
	//	$this->db->select(' DATE_FORMAT(tbl_booking_loads1.SiteOutDateTime,"%d-%m-%Y") as SiteOutDateTime ');
		...
		if (trim($StartDate) != "" && trim($EndDate) != "") {
			...
			$this->db->where_in('tbl_booking_loads1.Status', [4, 5, 6, 8]);
			$this->db->group_end();
			$this->db->group_end();
		}
	...
	public function GetLogs()
	{
		...
		if (!empty(trim($_POST['sort']))) {
			...
			if ($Sort[0] == 'ConveyanceNo') {
				$columnName = 'tbl_tickets.Conveyance ';
			}
			if ($Sort[0] == 'WaitTime') {
				$columnName = ' WaitTime ';
			}
			...
			if ($Sort[0] == 'DriverName') {
				$columnName = ' tbl_booking_loads1.DriverName ';
			}
			//if($Sort[0]=='Price'){ $columnName = ' tbl_booking1.Price '; } 
			if ($Sort[0] == 'Price') {
				$columnName = ' tbl_booking_loads1.LoadPrice ';
			}
			if ($Sort[0] == 'Status') {
				$columnName = ' tbl_booking_loads1.Status ';
			}
