...
class Chat_model extends CI_Model{ 
    ...
    public function GetDriverList(){
        ...
        $this->db->select('lorry_no');
        $this->db->from('tbl_driver_messages_new');
		$this->db->order_by('id DESC'); 
		$query = $this->db->get();
        $data = $query->result_array();
		$lorryNoMessages = array();
		foreach($data as $lorry){
			$lorryNoMessages[] = $lorry['lorry_no'];
		}
		$lorryNoMessages = array_unique($lorryNoMessages);
		...
		foreach($data as $key=>$dataInner){
			if(!in_array($dataInner['LorryNo'],$lorryNoMessages)){
				$lorryNoMessages[] = $dataInner['LorryNo'];
			}
		}
        ...
        $this->db->where("is_lorry_assigned","1");
        $this->db->where("AppUser","0");
		$lorryNoMessages = implode(',',$lorryNoMessages);
		$this->db->order_by('FIELD(`LorryNo`,'.$lorryNoMessages.")");
        $query = $this->db->get();
        $data = $query->result_array();
        ...
        foreach($data as $key=>$dataInner){
            ...
            $query = $this->db->query('SELECT id FROM tbl_driver_messages_new WHERE lorry_no = "'.$dataInner['LorryNo'].'" AND message_from="driver" AND (status="1" OR status="2")');
            $data[$key]['unread_messages'] = $query->num_rows();
        }
	...
	public function GetAllActiveDriverMessage(){ 
        ...
        $this->db->select('tbl_driver_messages_new.delivered_time');  
        $this->db->select('tbl_driver_messages_new.read_time');  
        $this->db->select('DATE_FORMAT(tbl_driver_messages_new.created_at,"%d-%m-%Y %T") as CreateDateTime');  	 	 
        $this->db->select('DATE_FORMAT(tbl_driver_messages_new.updated_at,"%d-%m-%Y  %T") as UpdateDateTime');  	 	   
