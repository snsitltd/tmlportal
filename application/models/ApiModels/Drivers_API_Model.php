...
class Drivers_API_model extends CI_Model
{
    ...
    function check_maintennce($lorry_no){
		$this->db->select('is_under_maintenance');    
        $this->db->from($this->userTbl);         
		$this->db->where('LorryNo',$lorry_no);
		$query = $this->db->get(); 
		$result = $query->row_array();
		if($result['is_under_maintenance']){
		    return true;
		}else{
		    return false;
		}
	} 
	function maintennce_message(){
	    return 'This lorry is under maintenance. Please contact the administrator.';
	}
