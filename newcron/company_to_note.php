<?php
	...
	$i = 0;  $j = 0;   $k = 0;   
	$q="select COMPANYID, NOTEID from  [Thames2013].[dbo].[TBL_COMPANY_NOTE]";
	$sql = sqlsrv_query($link, $q);
	while ( $ls = sqlsrv_fetch_array($sql, SQLSRV_FETCH_ASSOC)) {   
	    $k = $k+1;     
		$query = ""; $result = ""; $rows = ""; $insertQuery = ""; $result1 = ""; 
		$query = "select count(*) from tbl_company_to_note where CompanyID ='".trim($ls['COMPANYID'])."' 
		AND NoteID ='".trim($ls['NOTEID'])."'"; 
		$result = $mysqli -> query($query); 
		if($result){
			$rows = $result -> fetch_row();    
			if($rows[0]>0){  
				$updateQuery ="update `tbl_company_to_note` set `CompanyID` = '".$ls['COMPANYID']."', 
				`NoteID` = '".$mysqli -> real_escape_string(trim($ls['NOTEID']))."' 
				where CompanyID ='".$mysqli -> real_escape_string(trim($ls['COMPANYID']))."' ";
				$result2 = $mysqli -> query($updateQuery); 
				if($result2){ $j = $j+1; }  
			}else{
				$insertQuery ="INSERT INTO `tbl_company_to_note`(`CompanyID`, `NoteID` ) 
				values ('".$mysqli -> real_escape_string(trim($ls['COMPANYID']))."',
				'".$mysqli -> real_escape_string(trim($ls['NOTEID']))."')";
